...
struct Reach {
    ...
    uses_str: bool,             // any string literal reached (needs rt_str_lit)
}
...
/// What reachable code names in a type position - drives which class layouts,
/// buffer macros, and core representations (jim_str, jim_opt_*) get emitted.
#[derive(Default)]
struct Used {
    classes: HashSet<String>,
    bufs: BTreeMap<String, String>, // sanitized suffix -> element C type
    str_repr: bool,                 // jim_str appears in a type
    opt_structs: bool,              // some jim_opt_* struct appears in a type
}
...
fn walk_expr(e: &Expr, w: &mut Work) {
    match &e.kind {
        ...
        ExprKind::Str(_) => w.uses_str = true, // lowers to rt_str_lit
        ExprKind::Int(_)
        | ExprKind::Float(_)
        | ExprKind::CharLit(_)
        | ExprKind::Bool(_)
...
fn filter_runtime(src: &str, panic_abort: bool, blocks: &HashSet<&str>) -> String {
    ...
    for line in src.lines() {
        ...
        if let Some(rest) = t.strip_prefix("#ifdef ") {
            ...
            // Dropped blocks leave gaps; collapse runs of blank lines to one.
...
impl Cg {
    fn generate(&self, program: &Program) -> String {
        ...
        // What reachable code names in type position: which class layouts and
        // RawBuffer instantiations to emit, and whether jim_str / jim_opt_*
        // representations appear at all.
        let used = self.collect_used(program, &reach);
        ...
        let needs_opt = reach.uses_opt
            || used.opt_structs
            || blocks.contains("STRPARSE")
            || blocks.contains("IOFILE");
        ...
        if !used.bufs.is_empty() {
            blocks.insert("BUF");
        }
        if self.debug {
            blocks.insert("DEBUG");
        }
        // The arena: any constructor (objects are arena-allocated) or any block
        // whose helpers allocate. Allocation-free programs carry no arena.
        let needs_alloc = !reach.instantiated.is_empty()
            || ["OPT", "INT", "FLOAT", "BOOLCHAR", "STRING", "IOFILE", "BUF"]
                .iter()
                .any(|b| blocks.contains(b));
        if needs_alloc {
            blocks.insert("ALLOC");
        }
        // jim_str / rt_str_lit: a string literal, a String in type position,
        // the argv wrapper, or any block whose helpers traffic in jim_str.
        if reach.uses_str
            || used.str_repr
            || main_takes_argv
            || ["PANIC", "OPT", "INT", "FLOAT", "BOOLCHAR", "STRING", "STRPARSE", "IOPRINT",
                "IOFILE"]
                .iter()
                .any(|b| blocks.contains(b))
        {
        ...
        // rt_frame_top is shared by the panic handler and the debug frames.
        if blocks.contains("PANIC") || blocks.contains("DEBUG") {
            blocks.insert("FRAMETOP");
        }
        out.push_str(&filter_runtime(RUNTIME, self.panic_abort, &blocks));

        // ---- user class layouts (only the classes reachable code names) ----
        if !used.classes.is_empty() {
            out.push_str("\n/* ==== jim classes ==== */\n\n");
        }
        for c in &program.classes {
            if self.user_classes.contains(&c.name) && used.classes.contains(&c.name) {
        ...
        if main_takes_argv {
            ...
            // Nothing allocates: no arena to set up or sweep.
            out.push_str("int main(void) {\n    return (int)jim_user_main();\n}\n");
        }
        out
    }
    ...
    /// Everything reachable code names in a type position: the class layouts
    /// and RawBuffer instantiations to emit, plus whether the jim_str and
    /// jim_opt_* core representations appear at all. Restricting this to
    /// reachable declarations is what lets a program that never touches a
    /// container skip every container struct and buffer macro.
    fn collect_used(&self, program: &Program, reach: &Reach) -> Used {
        use std::collections::HashMap;
        let class_by: HashMap<&str, &ClassDecl> =
            program.classes.iter().map(|c| (c.name.as_str(), c)).collect();
        ...
        for c in &program.classes {
            for m in &c.methods {
                ...
                tys.push(Type::named(&c.name)); // the receiver
                for p in &m.params {
                    tys.push(p.ty.clone());
                }
            ...
            if reach.instantiated.contains(&c.name) {
                tys.push(Type::named(&c.name));
                if let Some(ct) = &c.ctor {
                    for p in &ct.params {
                        tys.push(p.ty.clone());
                    }
        ...
        for ty in &tys {
            self.note_type(ty, &mut pending, &mut used);
        }
        // An emitted layout names its field types, so close over fields.
        while let Some(key) = pending.pop() {
            if !used.classes.insert(key.clone()) {
                continue;
            }
            if let Some(c) = class_by.get(key.as_str()) {
                for fld in &c.fields {
                    self.note_type(&fld.ty, &mut pending, &mut used);
                }
            }
        }
        used
    ...
    /// Record what one type mention requires: a class layout, a RawBuffer
    /// instantiation, the jim_str representation, or a jim_opt_* struct.
    fn note_type(&self, ty: &Type, pending: &mut Vec<String>, used: &mut Used) {
        match ty {
            Type::Named(n) => {
                if n == "String" || n == "Exception" {
                    used.str_repr = true;
                } else if self.user_classes.contains(n) {
                    pending.push(n.clone());
                }
            }
            Type::Generic(n, p) => {
                if n == "RawBuffer" {
                    used.bufs.insert(c_name(&p.display()), self.ctype(p));
                } else {
                    pending.push(class_key_of(ty));
                }
                self.note_type(p, pending, used);
            }
            Type::Optional(p) => {
                if opt_is_struct(p) {
                    used.opt_structs = true;
                }
                self.note_type(p, pending, used);
            }
            Type::Pointer(p) => self.note_type(p, pending, used),
        }
    }
...
fn collect_types_in_block(b: &Block, out: &mut Vec<Type>) {
    for s in &b.stmts {
        match &s.kind {
            ...
            StmtKind::ForIn { var_ty, body, .. } => {
                out.push(var_ty.clone());
                collect_types_in_block(body, out);
            }
            StmtKind::TryCatch { body, var_ty, catch_body, .. } => {
                collect_types_in_block(body, out);
                out.push(var_ty.clone());
                collect_types_in_block(catch_body, out);
            }
